...
const BOOTSTRAP = String.raw`(function () {
  ...
  // 5) UI readiness: the redux store is created BEFORE the React UI finishes its first render.
  // Dispatching play/queue actions before the UI mounts crashes a mapStateToProps (reads .status of an
  // uninitialized slice) → permanent grey screen. Gate write/dispatch ops on this.
  try {
    const root = document.querySelector('#root');
    out.uiReady = !!(root && root.children && root.children.length > 0);
  } catch (e) { out.uiReady = false; }
...
/**
 * Run the bootstrap and make sure the store (and optionally the request factory + a fully rendered UI)
 * are available.
 * @param {boolean} needRequest  also require the signed Web-API request factory
 * @param {boolean} needUi       also require the React UI to have finished its first render. Pass true
 *   for any tool that DISPATCHES into the store — dispatching before the UI mounts crashes the renderer
 *   (grey screen). Read-only / pure-API tools don't need this.
 */
async function ensureBootstrapped(needRequest = false, needUi = false) {
  await ensureConnected();
  let status = await evalInPage(BOOTSTRAP);
  const unmet = (s) => !s.store || (needRequest && !s.request) || (needUi && !s.uiReady);
  if (unmet(status)) {
    // The page may still be loading; retry a couple of times.
    for (let i = 0; i < 5 && unmet(status); i++) {
  ...
  if (needUi && !status.uiReady) {
    throw new Error(
      "网易云界面尚未加载完成（仍在启动/渲染中）。请等几秒后【重试同一个工具一次】即可——不要反复重试，" +
        "也不要在加载中重复触发播放（会让界面异常/灰屏）。",
    );
  }
  return status;
}
...
async function dispatchAndState(type, payloadJson, waitMs = 350) {
  await ensureBootstrapped(false, true); // needUi: dispatching before the UI mounts greys out NCM
...
tool("toggle_play", "Toggle play/pause", "切换播放/暂停。基于【进度是否推进】检测真实状态后再显式播放或暂停（不依赖不可靠的 playingState）。", {},
  async () => {
    await ensureBootstrapped(false, true);
...
tool("play_song_by_id", "Play song by id", "Play any song by its NetEase song id (fetches detail, builds a play item, plays it immediately).",
  ...
  async ({ id }) => {
    await ensureBootstrapped(true, true);
